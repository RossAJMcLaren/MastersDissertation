...
def build_BB_RF():
    BB = RandomForestClassifier(n_estimators=1000, random_state=1)
    return BB
...
def training(epochs = 50, batch_size = 48):
    ...
    x_mal_train, x_mal_test, y_mal_train, y_mal_test = train_test_split(x_mal, y_mal, test_size = 0.20)
    x_ben_train, x_ben_test, y_ben_train, y_ben_test = train_test_split(x_ben, y_ben, test_size = 0.20)
    ...
    plot.ylabel("TPR")
    plot.legend()
    plot.savefig("Epoch_TPR({0},{1})RF.png")
    plot.show()
training()
