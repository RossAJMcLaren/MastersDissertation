...
def build_BB_MLP():
    BB = MLPClassifier(hidden_layer_sizes=(50,), max_iter=10, alpha=1e-4,
...
def training(epochs = 50, batch_size = 48):
    ...
    for run in range(epochs):
        for train in range(gen_x_mal_train.shape[0] // batch_size):
            ...
            i = np.random.randint(0, gen_x_mal_train.shape[0], batch_size)
            x_mal_set = gen_x_mal_train[i]
            noise = np.random.uniform(0, 1, size=(batch_size, 20))
    ...
    plot.ylabel("TPR")
    plot.legend()
    plot.savefig("Epoch_TPR({0},{1})MLP.png")
    plot.show()
training()